...
contract HopV2DirectTest is BaseTest {
    ...
    function send(uint32 _dstEid, bytes32 _to, uint256 _amount, uint256 _composeGas, bytes memory _composeMsg, uint128 _lzComposeValue) public {
        ...
        if (_composeMsg.length == 0) composeMsg = abi.encode(_to, _dstEid, 0, "");
        else {
    ...
    function test_sendDirect() public {
        setupBase();
        send(30110, 0x000000000000000000000000ef387fd954e0Fd6297f2bBA7996E9D79A0d826bC, 0, 400000, "Hello", 0.1E18);
    }
    ...
    function test_dstConfig() public {
        setupBase();
        logConfig(30101); // Ethereum
        logConfig(30102); // BSC
        logConfig(30106); // Avalanche
        logConfig(30110); // Arbitrum
        logConfig(30111); // Optimism
