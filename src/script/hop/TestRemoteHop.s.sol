...
contract TestRemoteHop is Script {
    ...
    function _ofts(uint256 chainId) internal pure returns (address[6] memory) {
        ...
        if (chainId == 1)
            return [ETH_FRXUSD_OFT, ETH_SFRXUSD_OFT, ETH_FRXETH_OFT, ETH_SFRXETH_OFT, ETH_WFRAX_OFT, ETH_FPI_OFT];
        if (chainId == 143)
            return [
                MONAD_FRXUSD_OFT,
                MONAD_SFRXUSD_OFT,
        ...
        if (chainId == 8_453)
            return [BASE_FRXUSD_OFT, BASE_SFRXUSD_OFT, BASE_FRXETH_OFT, BASE_SFRXETH_OFT, BASE_WFRAX_OFT, BASE_FPI_OFT];
        if (chainId == 59_144)
            return [
                LINEA_FRXUSD_OFT,
                LINEA_SFRXUSD_OFT,
        ...
        if (chainId == 324 || chainId == 2_741)
            return [ZK_FRXUSD_OFT, ZK_SFRXUSD_OFT, ZK_FRXETH_OFT, ZK_SFRXETH_OFT, ZK_WFRAX_OFT, ZK_FPI_OFT];
        return [
            SPOKE_FRXUSD_OFT,
            SPOKE_SFRXUSD_OFT,
            SPOKE_FRXETH_OFT,
