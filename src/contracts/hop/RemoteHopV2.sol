...
contract RemoteHopV2 is HopV2, IOAppComposer {
    ...
    function setNumDVNs(uint32 _numDVNs) public onlyAuthorized {
        RemoteHopV2Storage storage $ = _getRemoteHopV2Storage();
        $.numDVNs = _numDVNs;
    }

    function setHopFee(uint256 _hopFee) public onlyAuthorized {
        RemoteHopV2Storage storage $ = _getRemoteHopV2Storage();
        $.hopFee = _hopFee;
    }

    function setExecutorOptions(uint32 eid, bytes memory _options) public onlyAuthorized {
        RemoteHopV2Storage storage $ = _getRemoteHopV2Storage();
