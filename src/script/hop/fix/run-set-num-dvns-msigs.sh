...
usage() {
  echo "Usage: $0 [fresh]"
  echo
  echo "Generate direct local Safe JSON batches for legacy Hop setNumDVNs() into ${OUTPUT_DIR}."
  echo
  echo "Environment:"
  echo "  NUM_DVNS=5                 default numDVNs"
...
run_legacy_chain() {
  ...
  echo "SetLegacyHopNumDVNsDirect: ${chain_id} numDVNs=${num_dvns}"
